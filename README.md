...
| [machinery](https://github.com/RamXX/machinery) | Design substrate behind `design.machinery` (user opt-in, default `off`): domain model, C4 contract, state machines, oracles, and the deterministic gates `pvg gates`/`pvg rtm`/`pvg story approve-red` shell out to | `pvg update` installs the binary; `machinery install --home ~/.agents` places its skill |
